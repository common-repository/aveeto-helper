...
if (!class_exists('Aveeto_Autoload')) {

    class Aveeto_Autoload {
        ...
        static public function init_classes($classpath, $default_load_action, $params = array()) {
            $this_class = Aveeto_Autoload::getInstance();
            ...
            foreach ($result['delay_include'] as $action => $files) {
                ...
                if ('global' === $action) {
                    ...
                    $this_class->add_method($action . "_include", function () use (&$this_class, $action, $files) {
                        $include_array = array();
                        foreach ($files as $file) {
                            $tmp = explode("###", $file);
                            $include_array[$tmp[0]] = $tmp[1];
                    ...
                    add_action($action, array($this_class, $action . "_include"), 10);
                }
            }
                    ...
                    if (is_file($f)) {
                        ...
                        if ($file_info["extension"] == "php") {
                            ...
                            if (isset($file_data['include_action']) && $file_data['include_action']) {
                                $include_action = (!is_null(filter_var($file_data['include_action'], FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE))) ? (filter_var($file_data['include_action'], FILTER_VALIDATE_BOOLEAN)?self::DEFAULT_INCLIDE_ACTION:"aveeto_fake_action") : $file_data['include_action'];
